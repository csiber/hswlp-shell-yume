...
export default function MyFilesPage() {
  ...
  const fetcher = (url: string) =>
    fetch(url).then(
      (res) =>
        res.json() as Promise<{ items: UploadItem[]; albums: AlbumItem[] }>
    );
  ...
  const userCredits = useSessionStore(
    (s) => s.session?.user?.currentCredits ?? 0
  );
          ...
          item.mime?.startsWith("audio/") ? (
            ...
            >
              ...
              <MusicCard
                ...
                editTrigger={
                  item.moderation_status === "punished" ? undefined : (
                    <EditUploadDialog
                      upload={item}
                      onSaved={mutate}
                      trigger={
...
function PromptPreview({ url }: { url: string }) {
  const { data } = useSWR<string>(url, (u: string) =>

...
export async function POST(req: Request, { params }: RouteContext<{ id: string }>) {
  ...
  const upload = await env.DB.prepare('SELECT user_id FROM uploads WHERE id = ?1 LIMIT 1').bind(id).first<{ user_id: string }>()
  if (!upload) return new Response('Not found', { status: 404 })

  await env.DB.prepare('UPDATE uploads SET approved = 1, moderation_status = ?1, moderation_reason = ?2 WHERE id = ?3')

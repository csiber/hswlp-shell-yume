...
export async function POST(_req: NextRequest, { params }: RouteContext<{ id: string }>) {
  ...
  await env.DB.prepare('UPDATE uploads SET view_count = COALESCE(view_count,0) + 1 WHERE id = ?1')
    .bind(id)
    .run()
  const row = await env.DB.prepare('SELECT view_count, user_id FROM uploads WHERE id = ?1')
    .bind(id)
    .first<{ view_count: number; user_id: string }>()

  if (viewerId && row && row.user_id !== viewerId) {
    const existing = await env.DB.prepare('SELECT id, points_awarded FROM upload_rewards WHERE upload_id = ?1 AND viewer_id = ?2 AND event = ?3')
    ...
    if (!existing) {
      ...
      if (reward > 0) {
        await env.DB.prepare('UPDATE upload_rewards SET points_awarded = points_awarded + ?2 WHERE id = ?1')
